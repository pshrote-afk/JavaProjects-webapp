...
public class AddStudent extends HttpServlet {
  ...
  public void doPost(HttpServletRequest request, HttpServletResponse response) {
    ...
    try {
      ...
      boolean enrollmentNumberExists =
          studentManager.studentEnrollmentNumberExists(enrollmentNumber);
      boolean aadharCardNumberExists =
          studentManager.studentAadharCardNumberExists(aadharCardNumber);
      // enter if block if any of above three are problematic - send back form with filled data, and
      // error messages
      if (courseCodeExists == false
        ...
        if (enrollmentNumberExists == true)
          errorObject.addProperty(
              "enrollmentNumber", "Enrollment number already exists: " + enrollmentNumber);
        if (aadharCardNumberExists == true)
          errorObject.addProperty(
              "aadharCardNumber", "Aadhar card number already exists: " + aadharCardNumber);
      ...
      jsonObject.addProperty(
          "message", "Student with roll no " + student.getRollNo() + " successfully added!");
      ...
      jsonObject.add("success", new JsonPrimitive(false));
      jsonObject.addProperty(
          "message", "Error while adding student: " + blException.getGenericException());
      String jsonString = gson.toJson(jsonObject);
      pw.write(jsonString);
      pw.flush();
